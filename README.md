...
An interactive visual decision-tree builder and orchestration engine powered by Next.js 15, React Flow (@xyflow/react), Inngest Workflow Engine, and Google Gemini AI.
...
1. Graph Construction: The user adds decision nodes on the canvas and connects conditional outputs (YES green handle / NO red handle).
2. Event Dispatch: Clicking Execute Flow triggers a POST request to /api/execute-flow, sending node topologies and edge mappings to Inngest.
3. AI Step Processing: Inngest step functions evaluate node prompts via Google Gemini AI sequentially or conditionally based on prior decisions.
4. Execution Tracing: Results and status updates stream back to the UI execution logs panel and the Inngest Dev Server dashboard.
Getting Started
1. Prerequisites
